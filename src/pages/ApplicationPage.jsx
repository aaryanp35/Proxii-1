...
const SV_CHARACTERS = [
  'Gavin Belson', 'Big Head', 'Erlich Bachman', 'Richard Hendricks',
  'Gilfoyle', 'Dinesh', 'Jared', 'Monica', 'Russ Hanneman', 'Laurie Bream',
]

const REFERRAL_SOURCES = [
  'LinkedIn', 'Twitter / X', 'Friend or colleague', 'GitHub', 'Google search', 'Other',
]
...
function FieldError({ id, msg }) {
  return (
    <p id={id} role="alert" className="mt-1.5 text-xs font-bold text-rose-500 flex items-center gap-1">
...
function SectionHeader({ number, title, subtitle }) {
  return (
    <div className="flex items-start gap-3 sm:gap-4 mb-6">
      <span className="w-8 h-8 rounded-xl bg-slate-900 text-white text-sm font-black flex items-center justify-center flex-shrink-0 mt-0.5" aria-hidden="true">
        {number}
      </span>
      <div>
  ...
  return (
    <div>
      <label htmlFor={id} className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
        {label}{required && <span className="text-rose-400 ml-1" aria-label="required">*</span>}
      </label>
      <input
        id={id}
        type={type}
...
export function ApplicationPage() {
  ...
  } : { svCharacter: svError, iafRating: iafError, nailgun: nailgunError }
    ...
    try {
      ...
      const res = await fetch('/api/apply', {
        ...
        body: JSON.stringify({
          ...
          linkedin, portfolio,
          resumeName: resumeFile.name, resumePath: storagePath,
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-[#FAFAF8] flex flex-col">
        <Navbar />
        <main id="main-content" className="flex-1 flex items-center justify-center px-4 sm:px-6 py-16 sm:py-20">
          <div className="max-w-md w-full text-center">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-[#2D8E6F]/10 rounded-3xl flex items-center justify-center mx-auto mb-6" aria-hidden="true">
              <svg className="w-8 h-8 sm:w-10 sm:h-10 text-[#2D8E6F]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-3">Application submitted!</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                to="/careers"
                className="px-6 py-3 bg-slate-900 text-white text-sm font-bold rounded-2xl hover:bg-[#2D8E6F] transition-all focus-ring min-h-[52px] flex items-center justify-center"
              >
                Back to careers
              </Link>
              <Link
                to="/"
                className="px-6 py-3 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-2xl hover:border-slate-300 transition-all focus-ring min-h-[52px] flex items-center justify-center"
              >
                Go to dashboard
              </Link>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] flex flex-col">
      ...
      <main id="main-content" className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 md:px-12 py-8 md:py-10">
        {/* Back link */}
        <Link
          to={`/careers/${job.id}`}
        ...
        <div className="mb-8 md:mb-10">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Apply — {job.title}</h1>
          <p className="text-slate-500 mt-2 text-sm">{job.location} · {job.duration} · {job.department}</p>
        </div>

        <form onSubmit={handleSubmit} noValidate className="space-y-6 sm:space-y-8" aria-label="Job application form">

          {/* Section 1: Personal info */}
          <div className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-sm">
          ...
          <div className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-sm">
            ...
            <div className="space-y-4 sm:space-y-5">
              ...
              {/* Resume upload */}
              <div>
                <p className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5" id="resume-label">
                  Resume / CV <span className="text-rose-400" aria-label="required">*</span>
                </p>
                <label
                  className={`flex items-center gap-3 sm:gap-4 w-full border-2 border-dashed rounded-2xl p-4 sm:p-5 cursor-pointer transition-all min-h-[72px] ${
                    resumeFile ? 'border-[#2D8E6F]/40 bg-[#2D8E6F]/5' : errors.resumeFile ? 'border-rose-300 bg-rose-50/50' : 'border-slate-200 hover:border-[#2D8E6F]/40 hover:bg-slate-50'
                  }`}
                  aria-describedby={errors.resumeFile ? 'resume-error' : undefined}
                >
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0" aria-hidden="true">
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                {errors.resumeFile && <FieldError id="resume-error" msg={errors.resumeFile} />}
              </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              <div>
                <label htmlFor="availability" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Earliest start date <span className="text-rose-400" aria-label="required">*</span>
                </label>
                <input
                  id="availability"
                  type="date"
                ...
                />
                {errors.availability && <FieldError id="availability-error" msg={errors.availability} />}
              </div>

              <div>
                <label htmlFor="referral" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  How did you hear about us? <span className="text-rose-400" aria-label="required">*</span>
                </label>
                <select
                  id="referral"
                  value={referral}
                ...
                {errors.referral && <FieldError id="referral-error" msg={errors.referral} />}
              </div>
            </div>
          </div>

          {/* Section 4: Proxii screening */}
          <div className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 w-40 h-40 bg-[#2D8E6F]/4 rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none" aria-hidden="true" />
            <SectionHeader
              number="4"
            ...
            <div className="space-y-6 sm:space-y-7">
              ...
              <div>
                <label htmlFor="sv-character" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Who is the best Silicon Valley character? <span className="text-rose-400" aria-label="required">*</span>
                </label>
                <select
                  id="sv-character"
                  value={svCharacter}
                  ...
                  aria-required="true"
                  aria-invalid={!!(svError || (!svCharacter && errors.svCharacter))}
                  aria-describedby={svError || errors.svCharacter ? 'sv-character-error' : undefined}
                  className={`w-full bg-white border rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 transition-all appearance-none cursor-pointer ${
                    svCharacter ? 'text-slate-900' : 'text-slate-400'
                  } ${svError || (errors.svCharacter && !svCharacter) ? 'border-rose-300 focus:ring-rose-200' : 'border-slate-200 focus:ring-[#2D8E6F]/20'}`}
                >
                  <option value="" disabled>Select a character…</option>
                  {SV_CHARACTERS.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                {(svError || (!svError && errors.svCharacter)) && (
                  <FieldError id="sv-character-error" msg={svError || errors.svCharacter} />
                )}
              </div>

              {/* Indian Air Force rating */}
              <div>
                <label htmlFor="iaf-rating" className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Rate the Indian Air Force from 0 to 10 <span className="text-rose-400" aria-label="required">*</span>
                </label>
                <input
                  id="iaf-rating"
                  type="number"
                  ...
                  placeholder="e.g. 8.5"
                  aria-required="true"
                  aria-invalid={!!(iafError || (!iafRating && errors.iafRating))}
                  aria-describedby={iafError || errors.iafRating ? 'iaf-rating-error' : undefined}
                  className={`w-full bg-white border rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 transition-all placeholder:text-slate-400 text-slate-900 ${
                    iafError || (errors.iafRating && !iafRating) ? 'border-rose-300 focus:ring-rose-200' : 'border-slate-200 focus:ring-[#2D8E6F]/20'
                  }`}
                />
                {(iafError || (!iafError && errors.iafRating)) && (
                  <FieldError id="iaf-rating-error" msg={iafError || errors.iafRating} />
                )}
              </div>

              {/* Is the farzicle a NailGun */}
              <div>
                <p className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-3" id="nailgun-label">
                  Is the farzicle a NailGun? <span className="text-rose-400" aria-label="required">*</span>
                </p>
                <div className="flex gap-3" role="group" aria-labelledby="nailgun-label" aria-describedby={nailgunError || errors.nailgun ? 'nailgun-error' : undefined}>
                  {['Yes', 'No'].map(opt => (
                    <button
                      key={opt}
                      type="button"
                      aria-pressed={nailgun === opt}
                      onClick={() => setNailgun(opt)}
                      className={`flex-1 py-3 px-6 rounded-2xl text-sm font-bold border transition-all duration-200 focus-ring min-h-[52px] ${
                        nailgun === opt
                          ? opt === 'Yes'
                ...
                {(nailgunError || (!nailgunError && errors.nailgun && !nailgun)) && (
                  <FieldError id="nailgun-error" msg={nailgunError || errors.nailgun} />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-2 pb-8">
            {submitError && (
              <div role="alert" className="flex items-center gap-2 p-4 bg-rose-50 border border-rose-100 rounded-2xl text-sm text-rose-600 font-medium">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <p className="text-xs text-slate-400 font-medium">
                Fields marked <span className="text-rose-400 font-bold" aria-label="asterisk">*</span> are required.
              </p>
              <button
                type="submit"
                disabled={submitting}
                className="w-full sm:w-auto px-8 sm:px-10 py-3.5 bg-slate-900 text-white text-sm font-bold rounded-2xl hover:bg-[#2D8E6F] disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-300 shadow-lg shadow-slate-900/10 flex items-center justify-center gap-2 group focus-ring min-h-[52px]"
              >
                {submitting ? (
                  <>
